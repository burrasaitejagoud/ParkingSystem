...
public class CarEntryControllerTest {
    ...
    @Test
    public void givenNewlyStartedSession_whenExitParking_thenReturnMinimalFee() throws Exception {
        // Given
        String licenseNumber = "NEW123";
        CarEntry carEntry = new CarEntry();
        carEntry.setLicenseNumber(licenseNumber);
        carEntry.setStreetName("Java");
        carEntry.setStartTime(LocalDateTime.now()); // Just started
        carEntry.setEndTime(LocalDateTime.now());
        carEntry.setTotalFee(0L); // Assuming no fee for immediate exit
        ...
        mockMvc.perform(post("/api/parking/exit2")
                        ...
                        .content("{\"licenseNumber\":\"" + licenseNumber + "\"}"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Total fee: 0 cents")));
    }

    @Test
    public void givenServiceException_whenExitParking_thenReturnInternalServerError() throws Exception {
        // Given
        String licenseNumber = "ERROR123";

        given(carEntryService.endParkingSession(licenseNumber)).willThrow(new RuntimeException("Fee calculation error"));
        ...
        mockMvc.perform(post("/api/parking/exit2")
                        ...
                        .content("{\"licenseNumber\":\"" + licenseNumber + "\"}"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string(containsString("Fee calculation error")));
    }
}
